...
#                     进容器用 `docker exec -it grok-reg xai start -t 10` 等交互命令。
#                     PID 1 = tail -f /dev/null，所有 xai 后台 worker 会被 reaped。
#
#   run               `docker compose run --rm grok run` 一次性前台跑一次 batch。
#                     直接 exec xai --worker --target N，绕过 daemon fork（容器主进程即 worker）。
#
...
case "$mode" in
  idle)
    echo "[grok-docker] idle 模式 - 用 docker exec -it grok-reg xai ..."
    echo "[grok-docker]   例: docker exec -it grok-reg xai start -t 10"
    echo "[grok-docker]         docker exec -it grok-reg xai status"
    echo "[grok-docker]         docker exec -it grok-reg xai logs -f"
  ...
  *)
    # 直接把任意子命令转发给 xai：`docker run ... xai status`
    exec xai "$@"
    ;;
esac